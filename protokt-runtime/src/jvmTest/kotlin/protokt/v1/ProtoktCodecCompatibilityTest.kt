...
/**
 * Tests that ProtoktCodec produces byte-level correct protobuf wire format
 * by comparing against known encodings.
 */
@OptIn(OnlyForUseByGeneratedProtoCode::class)
class ProtoktCodecCompatibilityTest {
    @Test
    fun `varint encoding of 1`() {
        val buf = ByteArray(1)
        ProtoktWriter(buf).write(1)
    ...
    fun `fixed32 little-endian encoding`() {
        val buf = ByteArray(4)
        ProtoktWriter(buf).writeFixed32(0x01020304u)
    ...
    fun `fixed64 little-endian encoding`() {
        val buf = ByteArray(8)
        ProtoktWriter(buf).writeFixed64(0x0102030405060708uL)
    ...
    fun `sint32 zigzag encoding`() {
        ...
        for ((value, expected) in cases) {
            val buf = ByteArray(5)
            ProtoktWriter(buf).writeSInt32(value)
    ...
    fun `string encoding is length-prefixed UTF-8`() {
        val buf = ByteArray(8)
        ProtoktWriter(buf).write("testing")
