...
private class FileContentParser(
    ...
    fun parseContents(): List<TopLevelType> =
        enums.mapIndexed(::toEnum) +
            messages.mapIndexed(::toMessage) +
            services.mapIndexed(::toService)

    private fun toEnum(idx: Int, desc: EnumDescriptorProto): Enum {
    ...
    private fun toMessage(idx: Int, desc: DescriptorProto): Message {
        val typeName = desc.name
        val fieldList = toFields(desc)
    ...
    private fun toService(idx: Int, desc: ServiceDescriptorProto) =
        Service(
            name = desc.name,
    ...
    private fun toFields(desc: DescriptorProto): List<Field> {
    ...
    private fun mapEntry(fdp: FieldDescriptorProto) =
        if (fdp.label == LABEL_REPEATED && fdp.type == Type.TYPE_MESSAGE) {
            findMapEntry(ctx.fdp, fdp.typeName)
                ...
                ?.let { resolveMapEntry(toMessage(-1, it)) }
        } else {
            null
        }
    ...
    private fun requalifyProtoType(typeName: String): ClassName {
