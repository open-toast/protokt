                    ...
                    PropertySpec.builder("descriptor", ClassName(comToasttabProtoktV1, "FileDescriptor"))
                        .delegate(
                            buildCodeBlock {
                                ...
                                add("%T.buildFrom(\n", ClassName(comToasttabProtoktV1, "FileDescriptor"))
                                withIndent {
                                    add("descriptorData,\n")
                                    add("listOf(\n")
    ...
    private fun enumDescriptorExtensionProperties() =
        contents.types.flatMap { findEnums(emptyList(), it) }
            .map { (enum, containingTypes) ->
                PropertySpec.builder("descriptor", ClassName(comToasttabProtoktV1, "EnumDescriptor"))
    ...
    private fun messageDescriptorExtensionProperties() =
        contents.types.flatMap { findMessages(emptyList(), it) }
            .map { (msg, containingTypes) ->
                PropertySpec.builder("descriptor", ClassName(comToasttabProtoktV1, "Descriptor"))
