...
class Api private constructor(
    ...
    override fun serialize(serializer: com.toasttab.protokt.v1.KtMessageSerializer) {
        ...
        if (methods.isNotEmpty()) {
            methods.forEach { adapter.write(Tag(18)).write(it) }
        }
        if (options.isNotEmpty()) {
            options.forEach { adapter.write(Tag(26)).write(it) }
        }
        if (version.isNotEmpty()) {
            adapter.write(Tag(34)).write(version)
        }
        if (sourceContext != null) {
            adapter.write(Tag(42)).write(sourceContext)
        }
        if (mixins.isNotEmpty()) {
            mixins.forEach { adapter.write(Tag(50)).write(it) }
        }
        if (syntax.value != 0) {
            adapter.write(Tag(56)).write(syntax)
        }
...
class Method private constructor(
    ...
    override fun serialize(serializer: com.toasttab.protokt.v1.KtMessageSerializer) {
        ...
        if (requestTypeUrl.isNotEmpty()) {
            adapter.write(Tag(18)).write(requestTypeUrl)
        }
        if (requestStreaming) {
            adapter.write(Tag(24)).write(requestStreaming)
        }
        if (responseTypeUrl.isNotEmpty()) {
            adapter.write(Tag(34)).write(responseTypeUrl)
        }
        if (responseStreaming) {
            adapter.write(Tag(40)).write(responseStreaming)
        }
        if (options.isNotEmpty()) {
            options.forEach { adapter.write(Tag(50)).write(it) }
        }
        if (syntax.value != 0) {
            adapter.write(Tag(56)).write(syntax)
        }
...
class Mixin private constructor(
    ...
    override fun serialize(serializer: com.toasttab.protokt.v1.KtMessageSerializer) {
        ...
        if (root.isNotEmpty()) {
            adapter.write(Tag(18)).write(root)
        }
        adapter.writeUnknown(unknownFields)
    }
